...
public class NacosRegister implements IRegister {
    ...
    public void registerThreadPoolConfigList(List<ThreadPoolConfigEntity> threadPoolConfigEntityList) {
        ...
        try {
            String dataId = RedisRegistryEnum.THREAD_POOL_CONFIG_LIST_KEY.getKey() + ".yml";
            configService.publishConfig(dataId, "DEFAULT_GROUP", configContent.toString(), "yaml");
        } catch (NacosException e) {
            log.error("线程池配置列表注册到Nacos中-失败", e);
        }
    }
    ...
    public void registerThreadPoolConfig(ThreadPoolConfigEntity threadPoolConfigEntity) {
        String dataId = RedisRegistryEnum.THREAD_POOL_CONFIG_PARAMETER_LIST_KEY.getKey() + "-" + threadPoolConfigEntity.getAppName() + "-" + threadPoolConfigEntity.getThreadPoolName() + ".yml";
        StringBuilder configContent = new StringBuilder();
        Yaml yaml = new Yaml();
        // 创建一个 Map 来保存整个配置文档
        Map<String, ThreadPoolConfigEntity> configMap = new HashMap<>();
        ...
        try {
            ...
            log.error("线程池配置单个注册到Nacos中-失败", e);
        }
    }
    ...
    public void updateThreadPoolConfig(ThreadPoolConfigEntity threadPoolConfigEntity) {
        String dataId = RedisRegistryEnum.THREAD_POOL_CONFIG_PARAMETER_LIST_KEY.getKey() + "_" + threadPoolConfigEntity.getAppName() + "_" + threadPoolConfigEntity.getThreadPoolName();
        try {
            configService.publishConfig(dataId, "DEFAULT_GROUP", threadPoolConfigEntity.toString());
        } catch (NacosException e) {
            log.error("Failed to update thread pool config in Nacos", e);
        }
    }
    ...
    public ThreadPoolConfigEntity queryThreadPoolConfig(String appName, String threadPoolName) {
        try {
            String dataId = RedisRegistryEnum.THREAD_POOL_CONFIG_PARAMETER_LIST_KEY.getKey() + "-" + appName + "-" + threadPoolName + ".yml";
            // 启动的时候读取配置中心的配置
            String content = configService.getConfig(dataId, "DEFAULT_GROUP", 5000);
            Yaml yaml = new Yaml();
            Map<String, ThreadPoolConfigEntity> map = yaml.load(content);
